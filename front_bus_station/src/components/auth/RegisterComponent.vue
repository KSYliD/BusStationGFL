<template>
  <form @submit.prevent="submitRegister()"
    ...
    <input type="text" v-model="authData.firstName" required>
    <small id="nameErrors"
           class="error"
           v-if="errors.firstName.length>0">
      {{ errors.name.reduce((prev, curr) => prev + ", " + curr) }} </small>
    <label for="secondName">Second Name</label>
    <input type="text" v-model="authData.secondName" required>
    <small id="secondNameErrors"
           class="error"
           v-if="errors.secondName.length>0">
      {{ errors.secondName.reduce((prev, curr) => prev + ", " + curr) }} </small>
...
export default {
  ...
  methods: {
    ...
    async submitRegister() {
      ...
      try {
        const apiResponse = await axios.post('http://localhost:8080/api/v1/auth/register', data, {
          headers: {
